...
// 当数据库第一次被创建的时候，创建一个数据表，用以存储狗狗们的数据。
        onCreate: (db, version) {
          ...
          db.execute(
            "CREATE TABLE IF NOT EXISTS seeds(seed VARCHAR(128) PRIMARY KEY,mnemonic VARCHAR(256),private_keys JSON, address_count INT,is_default BOOLEAN,current_account INT)",
          );
          return db.execute(
